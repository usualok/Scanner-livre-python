# ========================================================================
# NOUVELLES FONCTIONS (ajoutées 2025-10-28)
# ========================================================================

def get_all_sales():
    """
    Retourne TOUTES les ventes de la table sales.
    
    Returns:
        list: Liste de toutes les ventes
    
    Utilisée par:
        - dashboard_module.py (calcul total items vendus, revenue)
        - sales_import_module.py (analytics, résumés)
    
    Exemple:
        >>> sales = get_all_sales()
        >>> print(f"Total ventes: {len(sales)}")
        Total ventes: 147
    """
...
# ========================================================================
# FONCTION 2/3: get_daily_scans(days)
# ========================================================================

def get_daily_scans(days=7):
    """
    Retourne le nombre de scans par jour pour les X derniers jours.
    
    Args:
        days: Nombre de jours à récupérer (défaut: 7)
    
    Returns:
        list: Liste avec date et quantité par jour
              [{'date': '2025-10-20', 'quantity': 50}, ...]
    
    Utilisée par:
        - dashboard_module.py (graphique scans quotidiens)
    
    Exemple:
        >>> data = get_daily_scans(7)
        >>> for day in data:
        ...     print(f"{day['date']}: {day['quantity']} scans")
        2025-10-27: 0
        2025-10-28: 2
    """
    ...
    results = fetch_all(query, (start_date.isoformat(),))
    
    # Remplir les jours manquants avec 0
    date_map = {row['date']: row['quantity'] for row in results}
    ...
    for i in range(days):
        ...
        complete_data.append({
            'date': date_str,
            'quantity': date_map.get(date_str, 0)
        })
...
# ========================================================================
# FONCTION 3/3: get_recent_scans(limit)
# ========================================================================

def get_recent_scans(limit=10):
    """
    Retourne les X derniers scans (les plus récents).
    
    Args:
        limit: Nombre maximum de scans à retourner (défaut: 10)
    
    Returns:
        list: Liste des derniers scans, triés du plus récent au plus ancien
    
    Utilisée par:
        - dashboard_module.py (afficher activité récente)
        - Onglet "Dashboard" (liste des derniers scans)
    
    Exemple:
        >>> scans = get_recent_scans(5)
        >>> for scan in scans:
        ...     print(f"{scan['timestamp']}: {scan['title']} ({scan['condition']})")
        2025-10-28 14:30:00: Harry Potter (USED)
        2025-10-28 14:28:00: Lord of the Rings (NEW)
    """
    query = """
    SELECT * FROM scans 
